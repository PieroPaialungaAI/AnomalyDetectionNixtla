...
def plot_timegpt_anomalies(df, ds_anomaly):
    ...
    plt.fill_between(x, timegpt_y_low, timegpt_y_high, color='blue', alpha=0.2, label='99% Confidence Interval')

    # Highlight anomalies
    match = df[df['ds'] == pd.to_datetime(ds_anomaly)]
    if not match.empty and match.iloc[0]['anomaly']:
        plt.scatter(match['ds'], match['y'], color='red', label='Anomaly Detected', zorder=5, s=30)
